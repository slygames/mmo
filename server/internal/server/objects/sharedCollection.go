...
//todo:needless use of ... as only one element is checked anyways. using 0 as a default capacity is better?
...
//todo:needless use of ... as only one element is checked anyways but can't use zero as a default value.. what to do?! 0 means no id was specified and real ids start from 1?
...
// Get the approximate number of objects in the map. This is approximate because the map is read without holding the lock.
func (s *SharedCollection[T]) Len() int {
	return len(s.objectsMap)
}

// Get the exact number of objects in the map. This is slower than Len() due to locking. Use this if needing exact length.
func (s *SharedCollection[T]) LenExact() int {
	s.mapMux.Lock()
	defer s.mapMux.Unlock()
